...
- **`kind: discussion` writer.** The kind is defined and renders correctly, but the *write* path (an admin form or CC slash command for "I just had this strategy chat, here's the summary") needs UX design. A CC slash command (`/log-chat <project>`) is the most lightweight option and probably what ships next.
...
- **Tier auto-detection in the Stop hook.** Currently the author types `:tier-1` / `:tier-2` in the commit subject. The hook could suggest based on positive-trigger output, but a misclassification is friction the author has to override. Deferred until I see real misclassification patterns.
...
- **Chat-capture browser/desktop helper.** Right now the `kind: discussion` writer assumes the human transcribes. A helper that exports a claude.ai conversation directly to an MDX draft would close the loop.
