...
`troubleshoot` · `tech-retro` · `ux-retro` · `decision` · `discussion` · `business` · `monetization` · `update` · `snapshot`. Definitions in `docs/project-log-spec.md` if a friend wants the system for their own repo.

- `decision` (ADR-style): the owner weighed multiple options and chose one. Required frontmatter: `status: "proposed" | "accepted" | "superseded"`, `reversibility: "easy" | "hard" | "one-way"`. Required body sections: Context · Options considered (with pros/cons each) · Chosen + Why · Trade-off accepted · Reversibility · Verified by · Discussion artifacts · Commit.
- `discussion`: a conversation (with Claude.ai chat, a person, brainstorm) that produced or surfaced options. Required frontmatter: `source: "<where>"`, `participants: ["..."]`, optional `linked_decision: "<filename>"`. Required body sections: Context · What we discussed · What I rejected from AI suggestion (if any) · Outcome · Why this matters · Next steps.
- `snapshot`: architecture overview or monthly state capture. Optional frontmatter: `period: "YYYY-MM"` for monthly snapshots.

## Presenting options to the human (non-negotiable)

When you (Claude Code) present multiple options to the user for them to choose, NEVER list options bare. Always include for **each** option:
...
## Decision tiers (auto-suggested by positive triggers)
...
Use `:tier-1` / `:tier-2` / `:tier-3` in the commit subject to override the auto-suggestion. Overrides require a one-line reason if downgrading from auto-T1 or upgrading from auto-T3.

`reversibility` values use Bezos two-way / one-way door framing: `two-way` (easy revert), `hard` (effortful revert), `one-way` (committed irreversibly).

## `kind: learning-gap` — record what you didn't initially understand

When the owner says "I didn't get that earlier" / re-asks a question in a different form / acknowledges a knowledge gap mid-conversation, propose a `kind: learning-gap` log entry in the same turn. Required slots:
...
Why this matters: anyone records what they learned. Almost no one records what they *didn't know* and the path their understanding took to arrive. That path is the AI engineer's growth artifact — a quarterly review of learning-gap entries reveals which topics deepened and which stayed shallow, and points at wiki entries that need updating to reflect current understanding.
